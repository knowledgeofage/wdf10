...
getbtn.addEventListener('click',function(e){
	...
	//console.log(`Please enter a number betweem ${min} to ${max}`)
	// message2.textContent = `Please enter a number betweem ${min} to ${max}`;
	setmessage2(`Please enter a number betweem ${min} to ${max}`,"red");
    }
...
function randomunm(min,max){
	let getrdm = Math.floor(Math.random()*(max-min)+min);
	return getrdm;
}

//console.log(randomunm);
